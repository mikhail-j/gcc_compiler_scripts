...
if test ! -f isl-0.14.1.tar.gz; then
	if test $found_curl_exit_code -eq 0; then
		curl -O http://isl.gforge.inria.fr/isl-0.14.1.tar.gz
		if test $? -ne 0; then
			exit 1
		fi
	else
...
if test -d isl-0.14.1; then
	...
	echo "error: The isl-0.14.1 folder does not exist!"
	exit 1
fi
